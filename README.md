...
本项目是基于 MkDocs Material 构建的静态文档站点，采用 Markdown + YAML Front Matter 作为内容载体，通过 GitHub Actions 实现自动化构建与部署。知识库覆盖法律法规、会计准则、审计准则、税法法规、行业知识、实务模板、案例库、工具方法、培训资源九大知识域（另含 00-元数据 系统域），当前已收录 395 项知识条目；实务模板、培训资源模块规划中（已建立模块骨架），法规原文类领域完成率 100%。
...
| `title` | 推荐 | 文档标题（索引页/方法类/行业类建议必填） | `企业会计准则第14号——收入` |
...
| 编码 | 文件名称 | 发文字号 | 状态 |
|------|----------|----------|------|
| LAW-01-001 | [中华人民共和国会计法](docs/01-法律法规/01-会计法律/中华人民共和国会计法.md) | 主席令第9号（2024修订） | 现行有效 |
